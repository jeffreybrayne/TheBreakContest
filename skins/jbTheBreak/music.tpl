{if jrUser_is_logged_in()}
	{assign var="quotaId" value="'$_user.profile_quota_id'"}
{else}
	{assign var="quotaId" value="1"}
{/if}
                    ...
                    <div class="row">
                        <div class="col12 last">
                            ...
                            <div class="body_1">
                                ...
                                {jrCore_array name="alpha" value=$_conf.jbTheBreak_letter_alphabet explode="true" separator=","}
                                ...
                                {if isset($_conf.jbTheBreak_require_images) && $_conf.jbTheBreak_require_images == 'on'}
                                    {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
                                        {jrCore_list module="jrAudio" order_by=$order_by search1="audio_genre like `$_post.search_string`" quota_id="'$quotaId'" template="music_row.tpl" require_image="audio_image" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                    {elseif !empty($_post._1)}
                                        {if isset($_post.option) && $_post.option == 'by_album'}
                                            {jrCore_list module="jrAudio" order_by=$order_by group_by="audio_album_url" template="music_row.tpl" quota_id="'$quotaId'" require_image="audio_image" search="audio_album like `$_post._1`%" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {else}
                                            {jrCore_list module="jrAudio" order_by=$order_by template="music_row.tpl" require_image="audio_image" quota_id="'$quotaId'" search="audio_title like `$_post._1`%" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {/if}
                                    {else}
                                        {if isset($_post.option) && $_post.option == 'by_album'}
                                            {jrCore_list module="jrAudio" order_by=$order_by group_by="audio_album_url" template="music_row.tpl" quota_id="'$quotaId'" require_image="audio_image" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {else}
                                            {jrCore_list module="jrAudio" order_by=$order_by template="music_row.tpl" require_image="audio_image" quota_id="'$quotaId'" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {/if}
                                    {/if}
                                {else}
                                    {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
                                        {jrCore_list module="jrAudio" order_by=$order_by search1="audio_genre like `$_post.search_string`" quota_id="'$quotaId'" template="music_row.tpl" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                    {elseif !empty($_post._1)}
                                        {if isset($_post.option) && $_post.option == 'by_album'}
                                            {jrCore_list module="jrAudio" order_by=$order_by group_by="audio_album_url" template="music_row.tpl" quota_id="'$quotaId'" search="audio_album like `$_post._1`%" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {else}
                                            {jrCore_list module="jrAudio" order_by=$order_by template="music_row.tpl" search="audio_title like `$_post._1`%" quota_id="'$quotaId'" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {/if}
                                    {else}
                                        {if isset($_post.option) && $_post.option == 'by_album'}
                                            {jrCore_list module="jrAudio" order_by=$order_by group_by="audio_album_url" template="music_row.tpl" quota_id="'$quotaId'" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {else}
                                            {jrCore_list module="jrAudio" order_by=$order_by template="music_row.tpl" quota_id="'$quotaId'" pagebreak=$_conf.jbTheBreak_default_pagebreak page=$_post.p}
                                        {/if}
                                    {/if}
                                {/if}
