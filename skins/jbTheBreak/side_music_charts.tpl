...
<table class="menu_tab">
    <tr>
        {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
            ...
            <td class="spacer">&nbsp;</td>
            <td>
                <div id="default" class="p_choice" onclick="jrLoad('#stats','{$jamroom_url}/music_chart_genres{if isset($period) && strlen($period) >0}/{$period}{/if}');jrSetActive('#default');">{jrCore_lang skin=$_conf.jrCore_active_skin id="182" default="Genres"}</div>
            </td>
        {else}
            <td>
            ...
            <td class="spacer">&nbsp;</td>
            <td>
                <div id="genre_search" class="p_choice" onclick="jrLoad('#stats','{$jamroom_url}/music_chart_genres{if isset($period) && strlen($period) >0}/{$period}{/if}');jrSetActive('#genre_search');">{jrCore_lang skin=$_conf.jrCore_active_skin id="182" default="Genres"}</div>
            </td>
        {/if}
    </tr>
</table>
...
<div id="stats" class="body_2 mb20">
    {if isset($_post.search_area) && $_post.search_area == 'audio_genre'}
        ...
        <br />
        <form class="margin" method="post" action="{$jamroom_url}/music_charts{if isset($period) && strlen($period) >0}_{$period}{/if}">
...
<div class="mb20 pt10">
    ...
    {if isset($_conf.jbTheBreak_featured_song) && strlen($_conf.jbTheBreak_featured_song) > 0}
        {jrCore_list module="jrAudio" order_by="_item_id desc" limit="1" search1="profile_active = 1" search2="_item_id = `$_conf.jbTheBreak_featured_song`" template=$featured_song}
    {else}
        {jrCore_list module="jrAudio" order_by="audio_file_stream_count numerical_desc" limit="1" search1="profile_active = 1" quota_id=$_conf.jbTheBreak_artist_quota template=$featured_song}
    {/if}
</div>
