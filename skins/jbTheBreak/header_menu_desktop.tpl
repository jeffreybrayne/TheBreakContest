...
{if $_conf.jrCore_maintenance_mode != 'on' || jrUser_is_master() || jrUser_is_admin()}
    ...
    <li>
        ...
        {if $_conf.jbTheBreak_artist_quota > 0 || $_conf.jbTheBreak_member_quota > 0}
            <ul>
                {if $_conf.jbTheBreak_artist_quota > 0}
                    <li><a href="{$jamroom_url}/artists">{jrCore_lang skin=$_conf.jrCore_active_skin id="48" default="artists"}</a></li>
                {/if}
                {if $_conf.jbTheBreak_member_quota > 0}
    ...
    {if isset($_conf.jbTheBreak_forum_profile_url) && strlen($_conf.jbTheBreak_forum_profile_url) > 0}
        {assign var="forum_url" value=$_conf.jbTheBreak_forum_profile_url}
    {/if}
    ...
    {if isset($_conf.jbTheBreak_docs_profile_url) && strlen($_conf.jbTheBreak_docs_profile_url) > 0}
        {assign var="doc_url" value=$_conf.jbTheBreak_docs_profile_url}
    {/if}
        ...
        <li>
            ...
            <ul>
                <li>
                    ...
                    <ul>
                        <li>
                            <a href="{$jamroom_url}/{$core_url}/{$url}">{jrCore_lang skin=$_conf.jrCore_active_skin id="91" default="Activity Logs"}</a>
                            <ul>
...
{if jrCore_module_is_active('jrFoxyCart') && strlen($_conf.jrFoxyCart_api_key) > 0}
    {if jrUser_is_logged_in()}
        <li>
            <a href="{$_conf.jrFoxyCart_store_domain}/cart?cart=view">{jrCore_image image="cart.png" width="24" height="24" alt="cart"}<span id="fc_minicart"><span id="fc_quantity" class="hl-4"></span></span></a>
        </li>
    {/if}
{/if}

{if jrCore_module_is_active('jrSearch')}
    <li><a onclick="jrSearch_modal_form();" title="Artist Search">{jrCore_image image="magnifying_glass.png" width="24" height="24" alt="Artist Search"}</a></li>
{/if}
</ul>
</nav>
